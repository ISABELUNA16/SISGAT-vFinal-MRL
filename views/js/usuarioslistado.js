function MostrarUsuarios(){
  ...
  $.ajax({
    type: "POST", 
    url: "controllers/BL/bl.usuarioslistado.php",
    dataType: 'JSON', 
    data: param, 
      ...
      if(data.data.length>0)
      {
        var table = $('#tblistadousuarios').DataTable({
          ...
          columns: [
            ...
            { title: 'Tipo de Usuario',     data: 'perfil'},
            { title: 'Fecha de Creación',   data: 'fechaCreacion'},
          ],
        });

        $('#tblistadousuarios tbody').on( 'click', 'button.editar', function () {
           var data = table.row( $(this).parents('tr') ).data();
           cargarDatosModal(data['id']);
        });  

        $('#tblistadousuarios tbody').on( 'click', 'button.eliminar', function () {
           var data = table.row( $(this).parents('tr') ).data();
           envioID(data['id']);
        });
